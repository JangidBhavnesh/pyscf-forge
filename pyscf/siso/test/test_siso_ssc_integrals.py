...
from pyscf.siso import sscint
from pyscf.siso.sscaddons import (ground_triplet_levels,
...
class KnownValues(unittest.TestCase):
    ...
    def test_spherical_components(self):
        ...
        spherical = sscint.cartesian_to_spherical(cart)
        reference = np.array([
            2.5 - 2.0j,
            3.0 - 5.0j,
            9.0 / np.sqrt(6.0),
        ...
        for iq, q in enumerate(sscint.SPHERICAL_COMPONENTS):
            jq = sscint.SPHERICAL_COMPONENTS.index(-q)
            np.testing.assert_allclose(
    ...
    def test_triplet_zfs_parameters(self):
        ...
        d_value, e_value, centered = triplet_zfs_parameters(levels)
    ...
    def test_ground_triplet_level_selection(self):
        ...
        levels, weights = ground_triplet_levels(
    ...
    def test_input_validation(self):
        ...
        with self.assertRaisesRegex(ValueError, 'leading shape'):
            sscint.cartesian_to_spherical(np.empty((2, 3)))
        with self.assertRaisesRegex(ValueError, 'three triplet'):
            triplet_zfs_parameters([0.0, 1.0])
        with self.assertRaisesRegex(ValueError, 'three integers'):
            ground_triplet_levels(np.eye(3), [0, 1])
